<template>
  <div class="text-overline mb-4">
    <span class="bebas-neue-regular" style="font-size: 1rem">Roles</span>
  </div>

  <a-button color="white" icon rounded="pill" density="compact" @click="roleEditOverlayVisible = !roleEditOverlayVisible" style="position: fixed; bottom: 150px; right: 10px; z-index: 100; height: 60px; width: 60px">
  ...
  <a-drawer v-model="roleEditOverlayVisible" class="justify-center bg-grey-darken-4" scrim="black" width="100%" style="overflow-y: scroll !important" persistent>
    ...
    <a-layout>
      <v-form>
        ...
        <v-select v-model="roleFormData.canViewAssignments" label="Can View Assignments" :items="assignments" multiple item-title="title" item-value="_id">
          <template v-slot:item="{ props, item }">
            <v-list-item v-bind="props" :subtitle="item.raw._id"></v-list-item>
          </template>
        </v-select>

        <v-select v-if="roleFormData.canViewAssignments.length > 0" v-model="roleFormData.canEditAssignments" label="Can Edit Assignments" :items="roleFormData.canViewAssignments" multiple item-title="title" item-value="_id"></v-select>

        <span>Schedule Viewer</span>
        <v-select v-model="roleFormData.canViewSchedules" label="Can View Schedules" :items="schedules" multiple></v-select>
        <v-select v-if="roleFormData.canViewSchedules.length > 0" v-model="roleFormData.canEditSchedules" label="Can Edit Assignments" :items="roleFormData.canViewSchedules" multiple></v-select>
